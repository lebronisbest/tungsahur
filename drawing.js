...
// --- New 2.5D Background and Objects ---
function drawIsoBox(obj) {
  const { x, width, height, depth } = obj;
  const baseY = GROUND_Y + PLAYER_HEIGHT; // ground level in world coords

  ctx.fillStyle = '#8B4513'; // front
  ctx.beginPath();
  ctx.moveTo(x, baseY);
  ctx.lineTo(x + depth, baseY - depth);
  ctx.lineTo(x + depth, baseY - depth - height);
  ctx.lineTo(x, baseY - height);
  ctx.closePath();
  ctx.fill();

  ctx.fillStyle = '#A0522D'; // top
  ctx.beginPath();
  ...
  ctx.fillStyle = '#CD853F'; // side
  ctx.beginPath();
  ctx.moveTo(x + width, baseY);
  ctx.lineTo(x + width + depth, baseY - depth);
  ctx.lineTo(x + width + depth, baseY - depth - height);
  ctx.lineTo(x + width, baseY - height);
  ctx.closePath();
  ctx.fill();
}
...
function drawPerspectiveBackground() {
  ...
  const horizon = canvas.height - 120;
  ctx.fillStyle = '#48BB78';
  ctx.beginPath();
  ctx.moveTo(-200, horizon);
  ctx.lineTo(WORLD_WIDTH + 200, horizon);
  ...
  for (let i = -400; i < WORLD_WIDTH + 400; i += 50) {
    ctx.beginPath();
    ctx.moveTo(i, canvas.height);
    ctx.lineTo(i + 400, horizon);
    ctx.stroke();
  }
